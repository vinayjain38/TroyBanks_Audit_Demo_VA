...
def tariff_audit_pipeline():
    ...
    @task(retries=1)
    def run_anomalies_task() -> None:
        cmd = [
            sys.executable,
            "/opt/project/src/va_step2_anomalies_db.py",
        ]
        result = subprocess.run(cmd, cwd=str(PROJECT_ROOT), text=True, capture_output=True)
        if result.returncode != 0:
            raise AirflowFailException(
                "va_step2_anomalies_db.py failed\n"
                f"STDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}"
            )
