...
# ==== CONFIGURATION ====
# Point this to your “step-1” base usage file:
# Use BASE_DIR + os.path.join for robust Windows paths
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
USAGE_PATH = os.path.join(BASE_DIR, "data", "va_step1_base.xlsx")

# Directory where your aggregated riders files live (folder containing riders files):
RIDERS_PATH = os.path.join(BASE_DIR, "data", "riders.xlsx")

# Where to drop the combined usage output file. Use a timestamped filename
# to avoid PermissionError when a previous file is open in Excel.
# TS = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_PATH = os.path.join(BASE_DIR, "data", f"usage_savings_output.xlsx")

schedule_path = os.path.join(BASE_DIR, "data", "Mini_Edit_VEPGA_Schedules_Compact.xlsx")

# Mapping: schedule ID → (subfolder name, processing function)
SCHEDULE_FUNCS = {}
...
def load_riders(path: str) -> pd.DataFrame:
    ...
    # rider amount column
    df['rider_total_per_kwh'] = pd.to_numeric(df['AGGREGATE RIDER ADJUSTMENT PER KWH'], errors='coerce').fillna(0.0)
    df['rider_total_per_kw'] = pd.to_numeric(df['AGGREGATE RIDER ADJUSTMENT PER KW'], errors='coerce').fillna(0.0)
...
def schedule_120(usage_df: pd.DataFrame, riders_df: pd.DataFrame) -> pd.DataFrame:
    ...
    schedule_120 = pd.read_excel(schedule_path := os.path.join(BASE_DIR, "data", "Mini_Edit_VEPGA_Schedules_Compact.xlsx"),
                                sheet_name="Schedule 120")
...
def schedule_102(usage_df: pd.DataFrame, riders_df: str = None) -> pd.DataFrame:
    ...
    schedule_102 = pd.read_excel(schedule_path := os.path.join(BASE_DIR, "data", "Mini_Edit_VEPGA_Schedules_Compact.xlsx"),
                                sheet_name="Schedule 102")
...
def schedule_100(usage_df: pd.DataFrame, riders_df: str = None) -> pd.DataFrame:
    ...
    schedule_100 = pd.read_excel(schedule_path := os.path.join(BASE_DIR, "data", "Mini_Edit_VEPGA_Schedules_Compact.xlsx"),
                                 sheet_name="Schedule 100")
...
def schedule_110(usage_df: pd.DataFrame, riders_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ---------------------------------------------------------
    schedule_path = os.path.join(BASE_DIR, "data", "Mini_Edit_VEPGA_Schedules_Compact.xlsx")
    sched = pd.read_excel(schedule_path, sheet_name="Schedule 110")

    # Safely convert "Rate / Description" → numeric ¢/kWh
    sched["Rate"] = (
        sched["Rate / Description"]
        .astype(str)
        .str.replace("¢", "", regex=False)
        .str.replace("/ kWh", "", regex=False)
        .str.replace("$", "", regex=False)
    ...
    # ---------------------------------------------------------
    nondemand_cust = find_rows(sched, "Non-Demand Billing", "Distribution", "Basic Customer Charge")
    demand_cust    = find_rows(sched, "Demand Billing", "Distribution", "Basic Customer Charge")
    ...
    nondemand_dist = find_rows(sched, "Non-Demand Billing", "Distribution", "Energy Charge")
    demand_dist    = find_rows(sched, "Demand Billing", "Distribution", "Energy Charge")
    ...
    ES_NONDEMAND_SUMMER = float(sched[(sched["Item"]=="Energy Charge (Summer)")]["Rate"].iloc[0])
    ES_NONDEMAND_BASE = float(sched[(sched["Item"]=="Energy Charge (Base)")]["Rate"].iloc[0]
)
    ...
    demand_es_summer_df = sched[
        (sched["Category"] == "Demand Billing") &
        (sched["Sub-Category"] == "ES Supply") &
        (sched["Item"].str.contains("Summer", case=False, na=False))
    ]

    demand_es_base_df = sched[
        (sched["Category"] == "Demand Billing") &
        (sched["Sub-Category"] == "ES Supply") &
        (sched["Item"].str.contains("Base", case=False, na=False))
    ]
