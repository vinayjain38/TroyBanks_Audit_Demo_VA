...
def upload_usage_data(file_path, batch_id=None, source_pdf=None, profile=None):
    ...
    if df["accountNumber"].isnull().all():
        # Use file-scoped fallback to avoid cross-file overwrite collisions.
        file_fallback_account = f"UNKNOWN_{file_obj.stem}"
        print(f"[WARNING] No Account Number found. Defaulting to '{file_fallback_account}'.")
        df["accountNumber"] = file_fallback_account
    if df["accountName"].isnull().all():
        df["accountName"] = f"Unknown Customer ({file_obj.stem})"
    ...
    df["source_pdf"] = source_pdf if source_pdf is not None else file_obj.name
    df["account_profile_json"] = json.dumps(profile, ensure_ascii=False) if profile else None

    # Same file content -> same batch_id: drop prior snapshot so re-upload replaces removed/changed rows.
    with engine.begin() as conn:
        conn.execute(
            text('DELETE FROM usage_bill WHERE batch_id = :bid'),
            {"bid": batch_id},
        )
