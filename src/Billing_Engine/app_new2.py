...
def load_usage(path: str) -> pd.DataFrame:
    ...
    if 'Total Consumption' in df.columns:
        ...
        if 'bill_period_end' in df.columns:
            df['bill_period_end'] = pd.to_datetime(df['bill_period_end'], errors='coerce', format='%m/%d/%y')
...
def schedule_102(usage_df: pd.DataFrame, riders_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'bill_period_end' in u.columns:
        u['bill_period_end'] = pd.to_datetime(u['bill_period_end'], errors='coerce', format='%Y-%m-%d')
...
def schedule_110(usage_df: pd.DataFrame, riders_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "bill_period_end" in u.columns:
        u["bill_period_end"] = pd.to_datetime(u["bill_period_end"], errors="coerce", format='%m/%d/%y')
