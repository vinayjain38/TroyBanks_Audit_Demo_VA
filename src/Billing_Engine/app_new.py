...
def schedule_120(usage_df: pd.DataFrame, riders_df: pd.DataFrame) -> pd.DataFrame:
    ...
    r120 = riders_df[riders_df["schedule_code"].str.contains("SCHEDULE 120", case=False, na=False)]
    if r120.empty:
...
def schedule_154(usage_df: pd.DataFrame, riders_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1. load riders (normalized)
    # riders = load_riders(RIDERS_PATH)
    r154 = riders_df[riders_df["schedule_code"].str.contains("SCHEDULE 154", case=False, na=False)]
    if r154.empty:
...
def schedule_102(usage_df: pd.DataFrame, riders_df: pd.DataFrame) -> pd.DataFrame:
    ...
    r102 = riders_df[riders_df["schedule_code"].str.contains("SCHEDULE 102", case=False, na=False)]
    if r102.empty:
